...
public class lemoine_omp_v2 {
    ...
    * @param currentValueOfQ   value of q used for verification
    *
    * @return -1 if the equation does not yield the objective number
    */
    public static int[] findConjecture (int objectiveNumber,
                                int currentValueOfP, int currentValueOfQ) {

        int[] result = {-1, currentValueOfQ};
        if ( Prime.isPrime(currentValueOfQ) &&
                                currentValueOfQ < objectiveNumber/2) {
    ...
    public static void conjecture() {
        ...
        for ( int objectiveNumber = lower_bound;
            ...
            if ( isOdd(objectiveNumber) ) {
                ...
                } while( result[0] != 1 );// As long as no result is found
